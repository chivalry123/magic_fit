...
def compressive_sensing_CE(energy_file, corr_in_file, eci_in_file,
                           ...
                           preserve_ground_state_range,preserve_ground_state_range_iterations,QP,concentrationmin,concentrationmax,researchonfittingmode,partitionsnumberforcvscore,DeactivateGSPreservation):
    ...
    if QP:
        ...
        for i in range(len(mus)):
            ...
            casm.perform_QP(mu=mu_now,concentrationmin=concentrationmin,
                           concentrationmax=concentrationmax,activate_GS_preservation=False)
        ...
        for i in range(len(mus)):
            mu_now=mus[i]
            casm.perform_QP(mu=mu_now,concentrationmin=concentrationmin,
                           concentrationmax=concentrationmax,activate_GS_preservation=True)
            idx_del = [i for i in range(casm.N_corr)
                       if i not in casm.nonzero_ECIs(eci_cutoff=eci_cutoff)]
            casm.ecis[idx_del] = 0.0
            E_all = casm.compute_ce_energies()
        ...
        for mu_now in mus:
            ...
            for i in range(len(partitioned_shuffled_grandlist)):
                ...
                sub_casm_now.perform_QP(mu=mu_now,concentrationmin=concentrationmin,
                               concentrationmax=concentrationmax,activate_GS_preservation=(not deactivate))
            ...
            print("mu_now, avg_out_of_sample_rmse is,",mu_now, avg_out_of_sample_rmse)

        print("after all the iterations, let's print mu and avg_out_of_sample_rmse")

        for mu_print,avg_print in zip(mus,avg_out_of_sample_rmse_list):
            print(mu_print," ",avg_print)
...
if (__name__ == "__main__"):
    ...
    compressive_sensing_CE(energy_file=args.energies,
