!----------------------------------------------------------------------!
! Original code:  Vidvuds Ozolins (UCLA Materials Science & Eng).      !
!                 June 19, 2012                                        !
! Structure weighting by Alexander Urban (AU) 2014-01-07               !
!----------------------------------------------------------------------!
program test_bregman
  ...
  !------------------ read sparse correlation matrix ------------------!
  ...
  allocate(Aval(nA), iAx(nA,2), En(Nstruc), ecis(Ncorr), &
           A(Nstruc,Ncorr), w(Nstruc))
  ...
  inquire(file='weights.dat', exist=fexists)
  if (fexists) then
     open(10, file='weights.dat', status='old', form='formatted')
     do i = 1, Nstruc
        read(10,*) w(i)
     end do
     close(10)
  ...
  ! construct full (weighted) correlation matrix
  A = 0d0
  do i = 1, nA
     A(iAx(i,1),iAx(i,2)) = Aval(i) * w(iAx(i,1))
  end do
...
!!$    deallocate( Apred, Epred, iApred)
    ...
    deallocate( Aval, iAx, En, ecis, A, w )
